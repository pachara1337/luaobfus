...
/_____/\____/\____/_____/_/ |_/ /_/ /_/ |_/___/\____/   
    ...
    std::cout << "Locate the folder:" << ENDL;
    std::string dir{};
    try {
        ...
        if (allFiles <= 0) {
            std::cout << "There is no file in the folder" << ENDL;
            try_exit();
            return 0;
        }
        ...
        std::cout << "Obfuscated " << allFiles << " file(s) in " << (newMS - startMS).count() << "ms." << ENDL;
        try_exit();
    }
    catch (const std::exception& exception) {
        std::cout << "ERROR: " << exception.what() << ENDL;
        std::cout << "Error locating the folder \"" + dir + "\"!" << ENDL;
...
void file_loop(std::string dir) {
    ...
    for (const auto& entry : it) {
        ...
        if (myfile.is_open())
        {
            ...
            std::cout << "(" << floor(completedFiles / allFiles * 100) << "%) Obfuscated " << folderPath << ENDL;
        }
    }
}
